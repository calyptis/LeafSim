...
def get_stylised_similar_cars_table(
    ...
    d = df_test.loc[
        car_to_explain_id,
        top_n_features + ["predicted_price"]
    ].to_dict()
    ...
    df_to_show = (
        df_to_show
        [top_n_features + ["similarity", "price"]]
        .style
        .format(formatting)
        ...
        .apply(
            ...
            cmap="Reds", df=df_to_show, col=f"diff_price", upper_threshold=1
        )
        .apply(
...
def get_similarity_plots(
    ...
    sns.lineplot(
        ...
        ci="sd", label="$\mu$ and $\sigma$ of prices of cars in training set", ax=ax
    )
